<nav class="flex justify-between items-center w-[92%]  mx-auto bg-white">
    ...
    {{-- <button class="bg-[#FF91B2] text-white px-7 py-2 rounded-[16px] hover:bg-[#9CC4B9]">Connexion</button> --}}
    <div>
        @if (Route::has('login'))
            <button class="">
                @auth
                    <a href="{{ url('/dashboard') }}"
                        class="bg-[#FF91B2] text-white px-7 py-2 rounded-[16px] hover:bg-[#9CC4B9]">Dashboard</a>
                @else
                    <a href="{{ route('login') }}"
                        class="bg-[#FF91B2] text-white px-7 py-2 rounded-[16px] hover:bg-[#9CC4B9]">Connexion</a>
                @endauth
            </button>
        @endif
